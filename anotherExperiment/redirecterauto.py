...
encoded = base64.b64encode(json.dumps(roles).encode()).decode()
...
for count in range(numberof):
    ...
    if instance_id:
        ...
        inner_cmd = (
            ...
            f"echo {shlex.quote(encoded)} | base64 -d > /tmp/roles.json && "
            f"cd /opt/myapp && /venv/bin/python3 redirecter.py < /tmp/roles.json > /tmp/last_run.log 2>&1; echo EXIT_CODE:$?"
        )
