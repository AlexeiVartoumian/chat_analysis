...
for count in range(numberof):
    ...
    if first_run == "true":
        ...
        scroller_worker = f"greencompany-{scroller_count}"
        scroller_count+=1
    ...
    if instance_id:
        ...
        inner_cmd = (
            ...
            f"echo {shlex.quote(encoded)} | base64 -d > /tmp/greenbyurls.json && "
            f"cd /opt/myapp && /venv/bin/python3 greenbycompany.py < /tmp/greenbyurls.json > /tmp/last_run.log 2>&1; echo EXIT_CODE:$?"
        )
