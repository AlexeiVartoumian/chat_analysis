...
if first_run == "yes":
    ...
    if instance_id:
        ...
        wait_for_ssm_online(ssm_client, instance_id)
        print(instance_id)
        search_term = shlex.quote(search_term)

        inner_cmd = (
            f"export DISPLAY=:1 && "
            f"export workflow_id={shlex.quote(workflow_id)} && "
            f"export search_term={shlex.quote(search_term)} && "
            f"export search_term_id={shlex.quote(search_term_id)} &&"
            f"cd /opt/myapp && /venv/bin/python3 screenshot.py > /tmp/last_run.log 2>&1; echo EXIT_CODE:$?"
        )
