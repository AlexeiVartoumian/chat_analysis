...
resp = client.list_objects(Bucket='output-store-green-cache-390746273208')

pages = paginator.paginate(Bucket='output-store-green-cache-390746273208')
...
with open("keys_green.txt" , "r" , encoding="utf-8") as f:
...
with open("keys_green.txt", "a" , encoding="utf-8") as f:
        
        for index , key  in enumerate(new_keys_to_upload):
            f.writelines(key)
            f.writelines("\n")

with open("newkeys_green.txt" , "w" , encoding="utf-8") as f :
...
with open("keys_ash.json" , "w" , encoding="utf-8" ) as f:
    ...
    for index , key  in enumerate(new_keys_to_upload):
        ...
        if document.startswith("companyGreen"):
...
for index ,key in enumerate(keys):
    ...
    with open (sanitizekey , "wb" ) as f :
        client.download_fileobj('output-store-green-cache-390746273208', key, f)
